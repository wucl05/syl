<template>
  <div class="outline-none"
    ...
    <div class="video-container flex flex-col lg:flex-row items-center justify-space-between lg:gap-10 gap-5">
      <nuxt-link :to="info.prev.id?`/video/${info.prev.id}`:''" :class="['order-3 lg:order-1 prev flex flex-shrink-0 items-center justify-start 2xl:w-96 xl:w-80 lg:w-60 w-full md:pl-10 md:pr-5 md:py-4 pl-4 lg:pl-12 pr-7 py-6 ease-in-out duration-300',info?.prev?.id? 'xl:hover:bg-primary-blue bg-primary-blue lg:bg-transparent cursor-pointer opacity-100' :'h-0 opacity-0']">
...
const video = ref()
loading.value = true
const res:LiveDetailResponseData<LiveDetail>  = await liveApi.videoDetail(id);
console.log('res===',res)
const {data = {}} = res
data.prev = data?.prev ?? {}
data.next = data?.next ?? {}
