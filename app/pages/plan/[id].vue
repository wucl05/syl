<template>
  <section v-if="info.data?.bgImg" class="flex flex-col bg-no-repeat bg-cover bg-center w-full aspect-[120/37.5] min-h-60 m-auto" :style="{'background-image': `url(${info.data?.bgImg ??''})`}">
    <div class="p-4 max-w-main m-auto w-full text-white text-left hidden lg:block">
      <h1 class="text-2xl sm:text-3xl md:text-4xl xl:text-7xl">{{ info.data?.title ?? '' }}</h1>
      <div class="mt-3 text-lg line-clamp-2">{{info.data?.introduction ?? ''}}</div>
  ...
  <section class="m-auto box-border lg:px-12 px-4 py-6 bg-white text-black">
    <h1 class="block lg:hidden font-semibold text-2xl sm:text-3xl md:text-4xl xl:text-7xl">{{ info.data?.title ?? '' }}</h1>
  ...
  <section class="bg-white bg-no-repeat bg-cover bg-center w-full lg:aspect-[120/47.06] m-auto text-center text-black" :style="{'background-image': `url(${bannerCard})`}">
    ...
    <p>{{ info.data?.advantageDesc ?? '' }}</p>
    <div v-if="info.data?.solutionAdvantageList?.length" class="gap-4 px-4 xl:px-0 flex items-center overflow-x-auto hide-scrollbar m-auto max-w-main mt-20">
      <div v-for="(item,index) in info.data.solutionAdvantageList" :key="index" class="card cursor-pointer w-[16.88rem] h-[21.25rem] box-border py-8 px-5 shrink-0 bg-white overflow-hidden relative">
  ...
  <section class="w-full max-w-main m-auto text-center text-black dark:text-white py-5 px-4 xl:px-0 lg:py-24">
    ...
    <img v-if="info?.data?.sysImg" :alt="planLang[locale]['plan:diagram']" :src="info?.data?.sysImg ?? ''" class="max-w-full object-cover m-auto block" />
  </section>
  <section class="w-full m-auto text-center bg-[#F6F7F9] text-black py-5 px-4 xl:px-0 lg:py-24">
    <div class="max-w-main m-auto">
      ...
      <UCarousel v-if="info?.data?.productList?.length"
        :items="info.data.productList"
        :ui="{
        ...
        <template #default="{ item }">
          <div class="w-full lg:px-3 box-bordermx-auto text-center">
            <div class="card-box bg-white px-3 hover:shadow-lg rounded-lg  transition delay-150 duration-300 ease-in-out">
              <img :src="item.coverImg" class="max-w-full m-auto block" draggable="false" />
      ...
      <UCarousel v-if="info?.data?.productArr?.length"
        :items="info.data.productArr"
        :ui="{
          item: 'basis-full',
          container: 'rounded-lg',
          indicators: {
        ...
        :indicators="info.data.productArr.length>1"
        :prev-button="{
          color: 'gray',
          icon: 'i-heroicons:chevron-left-20-solid',
          class: '-left-12'
        ...
        :arrows="info.data.productArr.length>1"
        ...
        <template #default="{ item }">
          <div class="lg:basis-1/3" v-for="(child,idx) in item" :key="idx">
              <div class="w-full lg:px-3 lg:pb-3 mx-auto text-center">
                <div class="card-box bg-white px-3 hover:shadow-lg rounded-lg  transition delay-150 duration-300 ease-in-out">
                  <img :src="child.coverImg" class="max-w-full m-auto block" draggable="false" />
                  <h3 class="mt-4 text-2xl font-semibold lineOne">
                  {{ child.name }}
                  </h3>
  ...
  <section :class="{'hidden':!info.data?.successCaseList?.length}" class="w-full m-auto text-center bg-white text-black py-5 px-4 xl:px-0 lg:py-24">
    <div class="max-w-main m-auto">
      ...
      <UCarousel v-if="info?.data?.successCaseList?.length"
        :items="info.data.successCaseList"
        :ui="{
          item: 'basis-full lg:basis-1/3',
          container: 'rounded-lg',
          indicators: {
        ...
        :indicators="info.data?.successCaseList?.length>1 && width<1024"
        :prev-button="{
          color: 'gray',
          icon: 'i-heroicons:chevron-left-20-solid',
        ...
        :arrows="info.data?.successCaseList?.length>1 && width<1024"
...
links.value[1].label = info.value.data?.title ?? ''
loading.value = false
