<template>
  <div class="cursor-pointer" :class="{'flex flex-col sm:flex-row gap-8':isFlex}" @click="handleClickItem(0)">
    ...
    <div :class="[isFlex?'flex-1 sm:flex-auto w-full sm:w-[76%] flex flex-col justify-between':'mt-4']">
      ...
      <div v-if="item.summary" class="text-sm mt-3 mb-1 flex-1 break-words line-clamp-4">{{ item.summary }}</div>
      <div class="text-sm text-[#6e6e6e] mt-1 shrink-0 flex justify-between items-center">
