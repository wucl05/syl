    ...
    useFetch(reqUrl, { ...options, key, headers: customHeaders })
      .then(({ data, error, refresh }) => {
        ...
        if (!value) {
          ...
          resolve({ ...value, refData: data, refresh });
        }
      })
      .catch((err) => {
...
export default class Http {
  get(url: string, params: Record<string, any> = {}, headers: Record<string, string> = {}, handleError: boolean = false, options = {}) {
    return fetch(url, { method: 'get', params, ...options }, headers, handleError);
  }

  post(url: string, params: Record<string, any> = {}, headers: Record<string, string> = {}, handleError: boolean = false, options = {}) {
    return fetch(
      url,
